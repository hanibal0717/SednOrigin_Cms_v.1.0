...
<%
			/**
			* @JSP Name : slidingBannerWrite.jsp
			* @Description : 슬라이딩 배너 등록/수정
...
if( bbsDataInfo == null ){
	modeType = "ins";
	actPage = "/banner/insertSlidingBanner.do";
	msg = "등록";
} else {
	
	if( !"reply".equals(modeType) && !"replyM".equals(modeType) ){ 
		modeType = "mod";
		actPage = "/banner/modifySlidingBanner.do";
		msg = "수정";
	}
...
function save(){
	...
	var f = document.frm; 
	f.action = "/banner/insertSlidingBanner.do";
	f.submit();
}
function remove(){
	if( $('#bnIdx').val() == '' ){
...
<form name="frm" id="frm" method="post">
	...
	<input type="hidden" id="bnGubun" name="bnGubun" value="01" />
	...
	<% } %>
		...
		<div class="row m-t-15 text-right m-b-10">
			<button id="search" class="btn-sm btn" style="" >목록</button>
		...
		<div class="table-responsive overflow" style="overflow: hidden; outline: none;">
			<table class="table table-bordered tile">
				...
				<tr>
					...
					<td colspan="3">
						<div class="col-md-12">
					<%=UvFileUtil.drawSingleUpload("BN", "C", "*.png; *.jpg;", "10000MB")%>
					<%
						if( bbsDataFile != null && bbsDataFile.size() > 0 ) {
							String rtn = "";
							...
							for (Map<String , Object> file : bbsDataFile) {
								if (StringUtil.nvl(file.get("fileType")).equals("IMG")) {
									rtn = rtn + "jsDrawImageFile('BN', '" + file.get("orgFileNm") + "', '" + file.get("fileNm") + "', " + file.get("FILE_SIZE") + ", " + file.get("dataIdx") + ", '" + IMG_URL + "', '" + file.get("'Y'")+ "');\n";	
								} else {
				...
				<tr>
					...
					<td colspan="3">
						<div class="col-md-12">
					<%=UvFileUtil.drawSingleUpload("BN_B", "B", "*.png; *.jpg;", "10000MB")%>
					<%
						if( bbsDataFile3 != null && bbsDataFile3.size() > 0 ) {
							String rtn = "";
							String IMG_URL = "/UPLOAD/BN_B/";
							for (Map<String , Object> file : bbsDataFile3) {
								if (StringUtil.nvl(file.get("fileType")).equals("IMG")) {
									rtn = rtn + "jsDrawImageFile('BN_B', '" + file.get("orgFileNm") + "', '" + file.get("fileNm") + "', " + file.get("FILE_SIZE") + ", " + file.get("dataIdx") + ", '" + IMG_URL + "', '" + file.get("'Y'")+ "');\n";	
								} else {
									rtn = rtn + "jsDrawFile('BN_B', '" + file.get("orgFileNm") + "', '" + file.get("fileNm") + "', " + file.get("FILE_SIZE") + ", '" + file.get("fileType") + "', " + file.get("dataIdx") + ");\n";
								}
							}
						%>
						<script type="text/javascript">
