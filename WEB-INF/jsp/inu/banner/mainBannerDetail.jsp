...
<%
			/**
			* @JSP Name : mainBannerDetail.jsp
			* @Description : 메인 슬라이드 배너 상세 페이지
...
function fn_modify(bn_idx){
	$("#modeType").val("mod");
	$("#bnIdx").val(bn_idx);
	$('#frm').attr('action' , '/banner/writeMainBanner.do');
	$('#frm').submit();
}
...
<form name="frm" id="frm" method="post" >
	...
	<div class="table-responsive overflow" style="overflow: hidden; outline: none;">
		<table class="table table-bordered tile">
			...
			<tr>
				<th scope="col">배너 이미지(Mobile)</th>
				<td colspan="3" class="image_size">
				<% if( bbsDataFile2 != null && bbsDataFile2.size() > 0 ) {%>
					<%
					Map<String , Object> bbsDataFileNode2;
					for(int i=0; i<bbsDataFile2.size(); i++){
						bbsDataFileNode2 = bbsDataFile2.get(i);
					%>
					<img src="/UPLOAD/BN_M/<%=bbsDataFileNode2.get("fileNm")%>">
					<%
					}
					%>
				<% }  %>
	...
	<div class="row m-t-15 text-right m-b-10">
		...
		<c:choose>
			<c:when test="${authResult == 'Y'}">
				...
				<button class="btn-sm btn" id="remove2">삭 제</buttonn>
			</c:when>
			<c:when test="${authResult == 'N'}">
			</c:when>
		</c:choose> 
