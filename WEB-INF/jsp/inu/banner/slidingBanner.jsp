...
function fn_view(dataIdx) {
	...
	var f = document.frm;
	$('#bnIdx').val(dataIdx);
	f.action = "/banner/detailSlidingBanner.do";
	f.submit();
}

function fn_page(pageNo) {
	$("#frm").attr("action","/banner/slidingBanner.do");
...
<form name="frm" id="frm" method="post" >
	...
	<div class="table-responsive" style="overflow: hidden; outline: none;">
		<table class="table table-bordered table-hover tile" id="data_tableW" >
			...
			%>
				<tr>
					<td colspan="7"><strong>해당 데이터가 존재하지 않습니다.</strong></td>
				</tr>
			<%
			}
...
%>
			<tr>
			 <td><%=rownum %></td>
			 <td>
				 <img src="/vod/file/getImage.do?data_idx=<%=dataIdx%>&file_gubun=BN" style="border:1px solid #cecece;cursor:pointer; width:45px; height:35px; padding-top:4px;" onclick="showImgPop('<%=dataIdx%>');" />
			 </td>
