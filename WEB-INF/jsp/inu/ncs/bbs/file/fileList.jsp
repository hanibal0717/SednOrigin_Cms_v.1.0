...
%>
<a href="#" onclick="javascript:fn_egov_downFile('<%=bbsMstIdx %>','<%=dataIdx %>','<%=attachIdx%>')"><%=orgFileNm %></a>
<%	 
		if( "Y".equals(updateFlag)){
%>		
			<img src="<c:url value='/images/icon/bu5_close.gif' />" 
				width="19" height="18" onClick="fn_egov_deleteFile('<%=bbsMstIdx %>','<%=dataIdx %>','<%=attachIdx%>');" alt="파일삭제" />
<%		
		}
%>
<br />
