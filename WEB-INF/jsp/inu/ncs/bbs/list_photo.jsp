...
<%						
				...
				if( list != null){
					...
					for(int i=0; i<list.size(); i++){
						...
						regNm     = StringUtil.nvl(bbsDataInfo.get("regNm"),"");
						privateYn = StringUtil.nvl(bbsDataInfo.get("privateYn"),"");
						attachIdx = StringUtil.nvl(bbsDataInfo.get("attachIdx"),""); 
...
%>
				<tr>
				 <td>${descNo }</td>
				 <td>
				 <img src="/bbs/dataFile/fileDown.do?bbs_mst_idx=<%=bbsMstIdx%>&data_idx=<%=dataIdx%>&attach_idx=<%=attachIdx%>" alt="" onerror="this.src='/bbs/images/tmp.jpg'" width="150" />
				 </td>
				 <td style="text-align:left">
				 	<a href="#" onclick="javascript:fn_view('<%=dataIdx%>');"><%= title%></a>
				 </td>
				 <td><%= regNm%></td>
				 <td><%= regDt%></td>
				 <td><%= hits%></td>
