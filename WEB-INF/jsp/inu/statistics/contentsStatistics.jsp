...
<%
			/**
			* @JSP Name : contentsStatistics.jsp
			* @Description : 컨텐츠별 통계
...
List<Map<String , Object>> list = (List)request.getAttribute("contentsStatList");
...
function loadList(value) {
	...
	frm.action = "/statistics/contentsStatistics.do";
	frm.submit();
}

function fn_page(pageNo) {
	$("#frm1").attr("action","/statistics/contentsStatistics.do");
...
<body>
<h2>컨텐츠별 통계<span class="text"></span></h2>
<p class="location"></p>
<p class="tar Mbo11">
</p>
<section>
			...
			<table class="data_tableW" id="data_tableW" summary="" >
				...
				if(list != null) {
					...
					for(int i=0; i<list.size(); i++) {
						...
						sConIdx = String.valueOf(contentDataInfo.get("sConIdx"));
						vType = StringUtil.nvl(contentDataInfo.get("vType"),"");
						title = StringUtil.nvl(contentDataInfo.get("title"),"");
						...
						regDt =  contentDataInfo.get("regDt").toString();
				%>
					<tr>
						<td><%=rownum%></td>
						<td><%=vType%></td>
