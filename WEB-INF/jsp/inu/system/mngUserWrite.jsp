...
function save(){
	if( $('#manageIdx').val() == '')
	{
		...
		if($('#managePwd').val() == '' || $('#managePwd').val() == null) {
			alert("비밀번호를 입력해주세요.");
			$('#managePwd').focus();
			return;
		}
	...
	if($('#manageNm').val() == '' || $('#manageNm').val() == null)
	{
		alert("이름을 입력해 주세요.");
		$('#manageNm').focus();
		return;
	...
	var f = document.frm; 
	f.action = "/system/insertMngUser.do";
	f.submit();
}
		...
		<p class="tar Mbo11">
			...
			<table class="data_tableW02">
				...
				<tr>
					...
					<td colspan="3">
						<%if(manageId != null && !"".equals(manageId)) {%>
							<%=manageId %>
							<input type="hidden" id="manageId" name="manageId" value="<%=manageId %>"/>
						<%} else { %>
							<input type="text" id="manageId" name="manageId"/>
						<%} %>
					</td>
				</tr>
					<%if(manageId != null && !"".equals(manageId)) {%>
					<tr>
						<!-- <th><span class="tx_red02">*</span>&nbsp;비밀번호</th>
						<td colspan="3">
							<input type="text" name="managePwd" id="managePwd" maxlength="12" value="" />
							<button id="passwordReset" class="btn_searchTbl06 prevent" style="">초기화</button>
						</td>
					</tr>-->
					<%} else { %>
						<tr>
							<th><span class="tx_red02">*</span>&nbsp;비밀번호</th>
							<td colspan="3"><input type="password" name="managePwd" id="managePwd" maxlength="12" value="" /></td>
						</tr>
					<%} %>
				<tr>
					<th><span class="tx_red02">*</span>이름</th>
					<td>
						<input type="text" id="manageNm" name="manageNm" value="<%=manageNm %>"/>
					</td>
				</tr>
				...
				<tr>
					<th>권한그룹</th>
					<td colspan="3">
						<select id="manageLevel" name="manageLevel">
							<c:forEach var="auth" items="${groupAuth }" varStatus="i">
								<option value="${auth.groupSeq }" <c:if test="${auth.groupSeq == bbsDataInfo.manageLevel}">selected=selected</c:if> >${auth.groupNm }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
