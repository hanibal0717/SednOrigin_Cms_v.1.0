...
function initCal(obj){
	...
	if(obj.fn_Obj) functionObj = obj.fn_Obj;
	
	// 최소년도, 최대년도 
	var minYear 	= obj.min == null ? "1960" : obj.min;
	...
	var iconUrl    = obj.iconUrl == null ? "/images/btn_cha.gif" : obj.iconUrl ; 
...
function calClickEvent(id){
	$("#" + id + " .calOtherDay,.calDay,.calSunDay,.calSatDay,.MonToday,.selMon,.dayEvMon").click(function(event) {
		...
		$("#" + id).hide();
		console.log('id:'+id)
		if(functionObj)functionObj(this.id.replace(id,""), id.substring(3));
	}).mouseover(function(){
...
function drawCalForm(id,tId,calType,min,max,todayYN,iconYN,imageUrl){ // tId = 입력창ID			
	if(iconYN == "y"){
		$(document.body).append("<div id='"+ id +"Icon' style='position:absolute;z-index: 10'><img style='z-index: -1;' id='"+ id +"IconImg' src='"+ imageUrl +"'></div>");
	}
...
function calPosition(target,id){
	...
	$("#" + target).css('width', '70px')
	if( tInput != null){
		$("#" + id).css({"top":tInput.top+tHeight , "left":tInput.left});
		//$("#" + id + "Icon").css({"top":tInput.top,"left":tInput.left+tWidth+2});
		//console.log('calpoid:'+id)
		//$("#" + id).css('width', '70px')
		//$("#" + id + "Icon").css({"top":tInput.top,"left":tInput.left+70+2})
	}
